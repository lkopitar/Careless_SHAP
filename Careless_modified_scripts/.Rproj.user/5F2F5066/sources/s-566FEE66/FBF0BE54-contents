...
data_mod_resp <- data_mod_all %>%
  dplyr::select(Careless, matches(paste0("^HE01_.*", items_r, "_r")),
                matches(paste0("^HE01_.*", items_nr)))

data_mod_rt <- data_mod_all %>%
  dplyr::select(Careless, starts_with("time_p"),
                starts_with("Careless"))

data_mod_dem <- data_mod_all %>%
  dplyr::select(Careless, starts_with(c("sex", "age", "SO")))

write.csv2(data_mod_all, file = paste0("data_mod_all.csv"), quote = FALSE, row.names = F)

write.csv2(data_mod_resp_rt, file = paste0("data_mod_resp_rt.csv"), quote = FALSE, row.names = F)
write.csv2(data_mod_resp_dem, file = paste0("data_mod_resp_dem.csv"), quote = FALSE, row.names = F)
write.csv2(data_mod_rt_dem, file = paste0("data_mod_rt_dem.csv"), quote = FALSE, row.names = F)

write.csv2(data_mod_resp, file = paste0("data_mod_resp.csv"), quote = FALSE, row.names = F)
write.csv2(data_mod_rt, file = paste0("data_mod_rt.csv"), quote = FALSE, row.names = F)
write.csv2(data_mod_dem, file = paste0("data_mod_dem.csv"), quote = FALSE, row.names = F)

write.csv2(data_mod_unrecoded, file = paste0("data_mod_unrecoded.csv"), quote = FALSE, row.names = F)
